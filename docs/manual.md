...
- Routes are road-following, generated by OSRM and cached on disk. They do not appear unless the trip has `waypoints` set. A trip with only a destination shows a straight line until you (or Research) provide intermediate cities.
...
**The route line isn't drawing.** The trip is probably missing `waypoints`. Add an inline array of cities along the route — `[Cleveland OH, Sandusky OH, Toledo OH]` — to the frontmatter and reload.
