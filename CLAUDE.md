...
`plan.md` and `candidates.md` are structured-YAML files with their own schemas (see `docs/superpowers/specs/2026-05-22-planning-plan-and-candidates-design.md`). They are written by `extractCandidates()` and edited by the Plan + Candidates UI; do not hand-edit their internal structure outside of the spec's defined fields.
...
- **Research →** (button on idea cards) — promotes an idea into a planning-stage folder with web-searched details: hours, prices, lodging, route highlights, logistics. Adds `waypoints` so the OSRM route line draws on the map. As a second leg chained into the same Ambient Background job, also runs `extractCandidates()` to write `plan.md` (day structure, field guide notes, gotchas) and `candidates.md` (stop + lodging candidate pool). Ambient Background: 60–120s, navigate away while it runs; cancel from the jobs drawer.
...
- When a module's behavior is unclear, **the test suite is authoritative.** AI adapters (`tests/ai-anthropic.test.js`, `tests/ai-openai.test.js`), candidate extraction (`tests/extract-candidates.test.js`), plan/candidates I/O (`tests/plan-io.test.js`, `tests/candidates-io.test.js`), brochure derivation (`tests/derive-brochure.test.js`), and frontmatter parsing (`tests/data-frontmatter.test.js`) document the contract more precisely than reading the implementation.
