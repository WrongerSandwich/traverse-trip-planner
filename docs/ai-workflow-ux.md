...
| **Server restart mid-job** | In-flight Ambient Background jobs are persisted to `.cache/.jobs.json` (the central registry written by `startJob` / `completeJob` / `failJob`). On boot, `sweepStaleJobs()` reads the registry, writes `last_run_error: 'interrupted'` + `last_run_error_at` + `last_run_message: 'Server restarted mid-job.'` to each affected trip's frontmatter, then deletes `.cache/.jobs.json`. A legacy scan also clears any pre-registry `running:` frontmatter flags from old installs, applying the same triple. The existing `lastRunFailed` derived store + trip-detail banner surfaces the `interrupted` code with the sentence "A previous research job was interrupted by a server restart. Try re-running it." and a "Retry" affordance — no new UI needed. In-memory short-action state is lost on restart but those actions complete fast enough that the user almost certainly already saw success or failure. See `docs/jobs-source-of-truth.md` for the full registry architecture. |
