...
A circular badge containing:
- Double-ring border (outer 1.5px, inner 0.5px hairline)
- Two-peak mountain silhouette, asymmetric, main peak left of center
- Sunset disc peeking behind the right shoulder of the main peak
- Subtle horizon line extending past the mountain base

**Canonical SVG** (designed for a forest background, mark drawn in bone):

```svg
<svg viewBox="0 0 80 80" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="Traverse">
  <circle cx="40" cy="40" r="36" fill="none" stroke="#EBE0C9" stroke-width="1.5"/>
  <circle cx="40" cy="40" r="33" fill="none" stroke="#EBE0C9" stroke-width="0.5"/>
  <circle cx="52" cy="36" r="7" fill="#D87B3F"/>
  <path d="M 14,52 L 24,38 L 32,44 L 42,22 L 56,40 L 66,52 Z" fill="#EBE0C9"/>
  <line x1="10" y1="52" x2="70" y2="52" stroke="#EBE0C9" stroke-width="0.5"/>
</svg>
```

**Geometry reference** (for any future tweaks):
- Mountain path: `M 14,52 L 24,38 L 32,44 L 42,22 L 56,40 L 66,52 Z` — two peaks with the smaller at (24, 38) and the main at (42, 22)
- Sun: circle at (52, 36) radius 7 — drawn _before_ the mountain so the mountain partially obscures it
- Horizon line: y=52, from x=10 to x=70
- Outer ring: r=36, stroke 1.5px; inner ring: r=33, stroke 0.5px (hairline detail)

### React component

A single parameterized component handles all four color variants. Uses `vectorEffect="non-scaling-stroke"` so stroke widths stay consistent regardless of display size.
...
const VARIANT_COLORS: Record<LogoVariant, { mark: string; sun: string }> = {
  primary:      { mark: '#1F4332', sun: '#D87B3F' },  // For light backgrounds
  inverse:      { mark: '#EBE0C9', sun: '#D87B3F' },  // For dark/forest backgrounds
  'mono-dark':  { mark: '#1F4332', sun: '#1F4332' },  // Single-color, dark
  'mono-light': { mark: '#EBE0C9', sun: '#EBE0C9' },  // Single-color, light
};
...
export const Logo = ({
  ...
  return (
    ...
    >
      ...
      <line x1="10" y1="52" x2="70" y2="52" stroke={c.mark} strokeWidth="0.5" vectorEffect="non-scaling-stroke" />
    </svg>
  );
};
```

For favicons at 16px, ship a separate simplified mark that drops the inner ring, horizon line, and detail (the badge ring, mountain, and sun only). The full mark is illegible below 24px.

### Lockup variations

**Horizontal lockup** — most common usage, in app headers and inline references:
...
**Stacked lockup** — for square contexts, app icons, splash screens:
...
**Mark only** — favicons, social profile pictures, decorative use.

**Hero / brand-moment lockup** — for splash, about, and marketing pages. Includes the orange hairlines flanking the wordmark and the tagline:
...
Use only in brand-moment treatments (hero lockup, about page, email footer). Don't repeat it in standard app headers — it's a treatment, not a perpetual subtitle.

### Usage rules

**Clear space.** Maintain a minimum of 1× mark radius on all sides of any lockup. For a 40px-wide mark, the radius is 20px, so the minimum clear space is 20px.

**Minimum size.** The full mark must be 24px or larger. Below 24px, the inner ring, sun, and horizon details become illegible — use a simplified favicon variant instead.

**Don'ts:**
- Don't recolor the mark with non-brand colors
