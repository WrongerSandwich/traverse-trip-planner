 ...
 * Looks for:  export const promise = { ... };
 * Returns the raw object text or null.
 */
function extractPromiseLiteral(source) {
  // Match export const promise = { ... }; allowing multi-line
  const m = source.match(/export\s+const\s+promise\s*=\s*(\{[\s\S]*?\});/);
  return m ? m[1] : null;
}
...
describe('promise-coverage', () => {
  it('every AI action route exports a valid promise object', async () => {
    ...
    for (const file of aiRoutes) {
      ...
      if (!raw) {
        errors.push(`${file}: missing "export const promise = {...}"`);
        continue;
      }
