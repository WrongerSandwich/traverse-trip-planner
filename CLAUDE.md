...
- **View brochure** (`/trips/<slug>/brochure` once `brochure.md` exists) — reached via `View full brochure ↗` in the `⋯` menu (Output group), or from the `View full brochure ↗` link at the top-right of the day-by-day itinerary view. Print-optimized layout: cover photos and hero meta (date · distance · drive time · duration), paper-map route inset from home, Stadia destination map with numbered pin overlay (edge indicators for off-viewport stops, "unmapped" tag for stops whose pin we couldn't geocode), and sectioned content (stops, lodging, field guide notes, gotchas). The brochure's `days` section renders in the day-by-day slot on the detail page and is the canonical print target.
...
- When a module's behavior is unclear, **the test suite is authoritative.** AI adapters (`tests/ai-anthropic.test.js`, `tests/ai-openai.test.js`), brochure extraction (`tests/brochure.test.js`), and frontmatter parsing (`tests/data-frontmatter.test.js`) document the contract more precisely than reading the implementation.
