...
- **Candidates section** (planning trips) — segmented Stops / Lodging browse rendered from `candidates.md`, with a category filter collapsed behind an opt-in "Filter" affordance (Stops only). Each card shows category, name, and a short description. Actions: "Promote to day…" (picks a target day and moves the candidate into the Plan), "Un-promote" (removes a stop from its day and returns it to the candidate pool). On completed trips renders read-only.
...
- **Today view** (`/trips/<slug>/today`) — phone-first, single-day render of a trip's plan (read-only on completed trips; read/write **In-trip capture** on planning — see below). Shows the day's stops (with hours, address, Navigate/Call/Site action buttons, and a collapsible tips & to-dos disclosure), tonight's lodging (Navigate + Booking), and a collapsed trip-wide field guide & gotchas panel. Day selection: a `?day=N` query param or auto-resolved to the current calendar date via `resolveCurrentDay()` (`src/lib/today.js`). Derived at request time from the same `deriveBrochure()` call the brochure uses — no AI, no cache. Shows an empty state ("No day-by-day plan yet") when `plan.yaml`/`candidates.yaml` are absent; never a 404 for a missing plan. Entry points: a "Today / in-trip view" link in the `itinerary-toolbar` affordance row alongside "↗ View brochure (for print)" (same `data.plan?.days?.length` gating), and a "Today view" entry in the `⋯` menu Output group alongside "View full brochure".
- **Save for offline** (Today view header + `⋯` menu Output group, when a plan exists) — downloads a single self-contained HTML file of the trip's Today view (all days, an in-file day switcher, `geo:`/`tel:`/Maps/Booking links, a "synced as of" banner). Works with zero connectivity to the home server: the deploy is plain-HTTP-on-LAN, so a downloadable bundle — not a PWA/service worker, which requires a secure context — is the offline path. Derived at request time by `renderOfflineToday()` (`src/lib/server/render-offline-today.js`) from the same `deriveBrochure()` state; **zero external subresources** (CI-enforced invariant). The PWA path is deferred, TLS-gated (#475).
- **In-trip capture** (Today view, planning stage only) — mark each stop **visited/skipped** (one-tap segmented toggles) and jot a per-stop **note** plus a per-day **note**, written live to the home server (Instant Inline, **online-only**). Stored as `status`/`note` on candidate stops in `candidates.yaml` and `log` on the plan day in `plan.yaml` (mirroring the `todos.done` mutable-field precedent; preserved across re-research by `realizePlan()`). Endpoints: `PATCH /api/capture/[slug]/stops/[id]` and `/api/capture/[slug]/days/[number]`. At Mark-as-completed the capture both **grounds the retro `chat()` prompt** (questions + notes) and is preserved **verbatim** under a `## In-trip notes` section in `notes.md`. Renders read-only on completed trips and in the offline bundle. Offline write-queue is deferred (#476).
...
The frontend reads trip data from the markdown files on each page load. All three external lookups are memoized in a single disk-backed file under `data/.cache/.caches.json` (shape `{ geo, image, route }`) and persist across restarts:

- `geo`   — Nominatim destination + waypoint coordinates
- `image` — Pexels photo URLs
- `route` — OSRM road route geometries

Collapsing the three former files (`.geocode-cache.json`, `.image-cache.json`, `.route-cache.json`) into one (#420) makes `flushCaches()` atomic across all three subsets so concurrent SSR requests can't leave the slices referentially out of sync. `data.js` carries a one-shot migration: on first read after upgrade, any legacy file is merged into the combined file and the legacy file is deleted.

A second disk-backed file, `data/.cache/.workflow-stats.json`, holds rolling p50 telemetry for the `_promise` time/token estimates (see `src/lib/server/workflow-stats.js`). It's written from `chat()` on every AI call and read by `getResolvedPromises()` at load time; the layout server passes the resolved map to the client as `data.promises`.

A third file, `data/.cache/.jobs.json`, is the volatile registry of in-flight Ambient Background jobs (written by `startJob` / `completeJob` / `failJob` in `src/lib/server/jobs.js`). On boot, `sweepStaleJobs()` reads it, marks each listed trip's frontmatter with `last_run_error: 'interrupted'`, then deletes the file. It lives under `.cache/` for the bind-mount reasons below but is registry state, not memoized lookups.
...
- For UX additions/changes, do a manual Playwright-MCP pass before merge — see [docs/manual-qa.md](docs/manual-qa.md). Drive it from the spec's **Manual QA pass** checklist; every UX-bearing spec should end with one. Exploratory, not a regression net.
